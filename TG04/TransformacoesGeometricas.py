...
def reshape(w,h):
    ...
    glOrtho(Min.x +3, Max.x +3, Min.y+3, Max.y+3, 0.0, 1.0)
...
def CarregaModelos():
    ...
    Modelos[0].leModelo("inimigo01.txt");
    Modelos.append(ModeloMatricial())
...
def CriaInstancias():
    ...
    Personagens[i].Direcao = Ponto(0.5,0) # direcao do movimento para a cima
    Personagens[i].Direcao.rotacionaZ(ang) # direcao alterada para a direita
...
def pode_atirar():
    ...
    # Se já passaram mais de 'intervalo' segundos, resetamos o contador
...
def CriaTiro2(nAtirador: int):
    if not pode_atirar():
        print("Limite de tiros atingido. Aguarde...")
        return
