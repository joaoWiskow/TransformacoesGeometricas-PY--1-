...
def CarregaModelos():
    ...
    Modelos[0].leModelo("inimigo03.txt");
    Modelos.append(ModeloMatricial())
...
def CriaTiro():
    ...
    # Pega os dois pontos da frente do envelope 
    envelope = Personagens[0].Envelope
    direcao = Personagens[i].Direcao

    # Ordena os pontos do envelope pela projeção na direção do disparo
    frente = sorted(envelope, key=lambda p: p.x * direcao.x + p.y * direcao.y, reverse=True)[:2]
    # Calcula o centro da frente
    centro_frente = (frente[0] + frente[1]) * 0.5

    # Posiciona o tiro um pouco à frente do personagem
    Personagens[i].Posicao = centro_frente + direcao * 0.3  # 0.3 pode ser ajustado

    # Aumenta a velocidade do tiro
    Personagens[i].Velocidade = 6  # velocidade maior
